...
public abstract class TorManager {
  ...
  public TorContext getOnionProxyContext() {
  ...
  public TorManager(final TorContext onionProxyContext, final boolean bootsrapManually) throws IOException {
  ...
  public synchronized void addBridgeLine(final String line) throws IOException {
    if (isBootstrapped()) {
  ...
  public synchronized Entry<String, TorEventHandler> publishHiddenService(final String hsDir,
    ...
    LOG.info("Creating hidden service " + hsDir);
    ...
    LOG.info("PUBLISH: Hidden service config has completed: " + Arrays.toString(config.toArray()));
  ...
  public synchronized void unpublishHiddenService(final String hsDir) throws IOException {
    ...
    LOG.info("UNPUBL Hidden service config has completed: " + Arrays.toString(conf.toArray()));
    controlConnection.setConf(conf);
    controlConnection.saveConf();
  }

  public synchronized boolean isHiddenServiceAvailable(final String onionurl) {
    try {
   ...
   *           - IO exceptions
   */
  public synchronized void enableNetwork(final boolean enable) throws IOException {
    if (controlConnection == null) {
      throw new RuntimeException("Tor is not running!");
    }
  ...
  public synchronized boolean isNetworkEnabled() throws IOException {
    ...
    final List<ConfigEntry> disableNetworkSettingValues = controlConnection.getConf(DISABLE_NETWORK);
  ...
  public void setBrigeLines(final String[] bridgeLines) throws IOException {
    if (bridgeLines == null) {
      return;
    }
